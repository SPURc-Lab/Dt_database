#!/usr/bin/python
# Usage python assemble_spliced.py <spliced.txt> from different species
# Python program to assemble the potential alternative splicing variant hits from different species
...
def get_geneid():
    ...
    for line in specFile1:
        if line.strip().split()[-1] in spec1_names:
            spec1_names[line.strip().split()[-1]] = spec1_names[line.strip().split()[-1]] + '\t' + line.split()[0]
        else:
            spec1_names[line.strip().split()[-1]] = line.split()[0]
    for line in specFile2:
        if line.strip().split()[-1] in spec2_names:
            spec2_names[line.strip().split()[-1]] = spec2_names[line.strip().split()[-1]] + '\t' + line.split()[0]
        else:
            spec2_names[line.strip().split()[-1]] = line.split()[0]
    for line in specFile3:
        if line.strip().split()[-1] in spec3_names:
            spec3_names[line.strip().split()[-1]] = spec3_names[line.strip().split()[-1]] + '\t' + line.split()[0]
        else:
            spec3_names[line.strip().split()[-1]] = line.split()[0]
    for line in specFile4:
        if line.strip().split()[-1] in spec4_names:
            spec4_names[line.strip().split()[-1]] = spec4_names[line.strip().split()[-1]] + '\t' + line.split()[0]
        else:
